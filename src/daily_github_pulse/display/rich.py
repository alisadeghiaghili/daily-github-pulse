...
def print_repo_table(repos_by_category: dict, snapshots: dict) -> None:
    ...
    from daily_github_pulse.core.velocity import star_delta, daily_velocity
...
def _print_repo_plain(repo: dict, rank: int, snapshots: dict) -> None:
    ...
    from daily_github_pulse.display.plain import format_velocity
    delta    = star_delta(repo, snapshots)
    velocity = daily_velocity(repo, snapshots)
    print(
        f"{'=' * 70}\n"
...
def print_developer_table(developers: list[dict]) -> None:
    ...
    """
    if not RICH_AVAILABLE:
        from daily_github_pulse.display.plain import format_developer
    ...
    from daily_github_pulse.forges.base import ForgeUser as _ForgeUser
